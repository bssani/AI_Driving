...
`/RacingAI/Maps/TestOval`은 길이 697m의 닫힌 오벌이고 `TrackHalfWidth`는 700cm다.
스플라인 좌우 양끝을 따라 벽 200개가 세워져 있다(라벨 `TESTWALL_*`, 태그 `TESTWALL`).
벽 없이 재면 AI가 코스를 벗어나도 아무 일이 없어서, 조향이 맞는지 아닌지가 드러나지 않는다.

실측(2026-09-10, 5랩, 포그라운드): Pro 134.81초 완주, Advanced 4랩, Rookie 3랩,
재배치 0회, 후진 탈출 0회. 완주한 Pro의 최종 좌우 오프셋은 5cm로 라인 위였다.

벽을 지우려면 태그 `TESTWALL`로 골라 지운다.

## 차량을 바꾸면 반드시 재는 것

프로파일(`DA_Driver_*`)의 `MinTurnRadius`, `LateralAccelBudget`, `BrakingDecel`은 난이도가
아니라 **그 차량의 물리적 한계**다. 차를 바꾸면 그대로 다른 차의 값이 되고, 증상은
"AI가 느리다" 또는 "AI가 코너 바깥으로 밀려 벽을 긁는다"로 나타나 원인이 AI에 있는 것처럼
보인다. 콘솔에서 잰다.

```
racing.Calibrate 1      -- 시작. 직선 풀 스로틀 -> 풀 브레이크 -> 풀락 선회
...
초당 여덟 번의 충돌음이 됐다. `ImpactEscalation`(이번 접촉에서 가장 컸던 것보다 이 배수
이상 커야 다시 울림)과 `ContactReleaseTime`(이만큼 끊겨야 새 접촉)으로 묶는다.
실측(벽 긁기 40초): 손보기 전 6회, 손본 뒤 2회.

**들을 만한 것과 볼 만한 것의 기준은 다르다.** 충돌음은 면으로 파고드는 속도로 판정하는
것이 맞지만, 불꽃은 짓눌림이 아니라 갈림에서 난다. 스파크를 같은 기준으로 두면 벽을
스치는 차는 아무것도 못 낸다. 벽을 따라 미끄러질 때 닫힘 속도는 거의 0이기 때문이다.
그러면 정면으로 받는 것만 튀게 되어 사실상 차끼리 부딪힐 때만 난다.
`UVehicleImpactFXComponent`는 접선 속도(`MinScrapeSpeed`)도 함께 본다. 실측(벽 긁기 40초):
기준을 고치기 전 5개, 고친 뒤 67개가 벽을 따라 이어졌고 세기는 속도에 따라 0.33에서 0으로
줄었다. 깨끗하게 달린 AI 세 대는 0개였다. 초당 개수는 `ScrapeInterval`로 조절한다.
