...
public:
	...
	/**
	 * 전방 주시선이 감쌀 수 있는 최대 회전각 (도). 0이면 끔.
	 *
	 * pure pursuit은 목표점까지 직선으로 겨눕니다. 그래서 코너에서 목표점이 멀면
	 * 그 직선이 곡선 안쪽을 가로지릅니다. 속도가 오를수록 주시 거리가 길어지므로
	 * 빠른 AI일수록 코너를 크게 잘라 먹고, 안쪽 벽을 긁으며 통과합니다.
	 * 프로파일을 빠르게 만들면 반드시 같이 나타나는 증상입니다.
	 *
	 * 목표점이 코스를 따라 몇 도나 돌아간 곳인지로 주시 거리를 제한하면 이 문제가
	 * 사라집니다. 30도쯤이면 직선에서는 아무 영향이 없고 코너에서만 짧아집니다.
	 * 코너에서 안쪽으로 파고들면 줄이고, 반응이 굼뜨면 늘리세요.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Steering", meta = (ClampMin = "0.0", ClampMax = "90.0", Units = "deg"))
	float MaxLookaheadAngleDegrees = 30.f;
